...
- **Auto-Punctuation**: Intelligent comma insertion before conjunctions (*но, а, который, because, although*).
- **Question Logic**: Automatic `?` detection based on sentence-starting tokens (*кто, почему, how, when*).
...
- **Speech-to-Clean-Thought**: Removes disfluencies (*um, uh, э-э*) while strictly preserving the original language.

### **C. OS Synergy**
- **Context-Awareness**: Detects active windows (*VS Code, IntelliJ, Cursor, Neovim*) to adjust formatting rules (e.g., stripping periods in code editors).
...
---
*Created by Gemini CLI for the SOVRN v3.3 Architecture.*
